...
const CalendarModal = ({show, onClose, cityName}) => {
    ...
    const formatSelectedRange = () => {
        ...
        if (selectedRange.length === 2) {
            return `${formatDate(selectedRange[0])} ~ ${formatDate(selectedRange[1])}`;
        }
            ...
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="calendar">
                    ...
                    {calendarDays.map((item, index) => {
                        ...
                        // 선택된 범위의 시작 날짜인지 확인
                        const isStart = selectedRange.length > 0 && dateToCheck.getTime() === selectedRange[0].getTime();

                        // 선택된 범위의 끝 날짜인지 확인
                        const isEnd = selectedRange.length === 2 && dateToCheck.getTime() === selectedRange[1].getTime();
